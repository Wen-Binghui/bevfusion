...
def _eval_map(
    ...
    for i, clsname in enumerate(class_name):
        ...
        # at the top level.
        tpfp_fn = partial(
            tpfp_fn, threshold=threshold, metric=metric, coord_dim=coord_dim
        )
        args = []
        ...
        print(
            "cls:{} done in {:2f}s!!".format(clsname, float(timer.since_last_check()))
        )
    pool.close()
    aps = []
    ...
    print_map_summary(mean_ap, eval_results, class_name=class_name, logger=logger)
...
def print_map_summary(
    mean_ap, results, class_name=None, scale_ranges=None, logger=None
):
